...
describe("bundle plugin hooks", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.TINKERCLAW_BUNDLED_HOOKS_DIR;
    process.env.TINKERCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.TINKERCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.TINKERCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
