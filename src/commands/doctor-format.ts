...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.TINKERCLAW_PROFILE);
    hints.push(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        platform,
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.TINKERCLAW_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.TINKERCLAW_PROFILE),
      }),
    );
  }
  return hints;
