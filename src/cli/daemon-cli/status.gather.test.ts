...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    TINKERCLAW_STATE_DIR: "/tmp/openclaw-daemon",
    TINKERCLAW_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.TINKERCLAW_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.TINKERCLAW_CONFIG_PATH ?? `${stateDir}/openclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.TINKERCLAW_STATE_DIR = "/tmp/openclaw-cli";
    process.env.TINKERCLAW_CONFIG_PATH = "/tmp/openclaw-cli/openclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.TINKERCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.TINKERCLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    ...
    process.env.TINKERCLAW_STATE_DIR = tmp;
    process.env.TINKERCLAW_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        TINKERCLAW_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.TINKERCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
