...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "TINKERCLAW_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "TINKERCLAW_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "TINKERCLAW_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "TINKERCLAW_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.openclaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        process.env.TINKERCLAW_STATE_DIR = defaultStateDir;
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "TINKERCLAW_STATE_DIR=./evil-state",
            "TINKERCLAW_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.TINKERCLAW_STATE_DIR).toBe(stateDir);
        expect(process.env.TINKERCLAW_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "TINKERCLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "TINKERCLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "TINKERCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "TINKERCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "TINKERCLAW_GATEWAY_TOKEN=attacker-token",
            "TINKERCLAW_GATEWAY_PASSWORD=attacker-password",
            "TINKERCLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.TINKERCLAW_STATE_DIR).toBeUndefined();
  ...
  it("blocks path-override vars (TINKERCLAW_AGENT_DIR, TINKERCLAW_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, TINKERCLAW_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        expect(process.env.TINKERCLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.TINKERCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.TINKERCLAW_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks TINKERCLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "TINKERCLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.TINKERCLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.TINKERCLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "TINKERCLAW_PINNED_PYTHON=./attacker-python",
            "TINKERCLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.TINKERCLAW_PINNED_PYTHON;
        delete process.env.TINKERCLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.TINKERCLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.TINKERCLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "TINKERCLAW_PINNED_PYTHON=/trusted/python",
            "TINKERCLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.TINKERCLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.TINKERCLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "TINKERCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "TINKERCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "TINKERCLAW_GATEWAY_TOKEN=trusted-token",
            "TINKERCLAW_GATEWAY_PASSWORD=trusted-password",
            "TINKERCLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.TINKERCLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.TINKERCLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.TINKERCLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.TINKERCLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.TINKERCLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.TINKERCLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.TINKERCLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "TINKERCLAW_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks TINKERCLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "TINKERCLAW_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when TINKERCLAW_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.TINKERCLAW_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "openclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.TINKERCLAW_STATE_DIR).toBe(customStateDir);
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          "TINKERCLAW_GIT_DIR",
          "TINKERCLAW_WORKSPACE_DIR",
          "TINKERCLAW_MDNS_HOSTNAME",
          "TINKERCLAW_SESSION_CACHE_TTL_MS",
          "TINKERCLAW_UPDATE_PACKAGE_SPEC",
          "TINKERCLAW_GATEWAY_PORT",
          "TINKERCLAW_GATEWAY_URL",
          "TINKERCLAW_CLAWHUB_URL",
          "CLAWHUB_URL",
          "TINKERCLAW_CLAWHUB_TOKEN",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
