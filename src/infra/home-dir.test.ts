...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers TINKERCLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for TINKERCLAW_HOME",
      env: { TINKERCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when TINKERCLAW_HOME is tilde-only and no fallback home exists",
      env: { TINKERCLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores TINKERCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          TINKERCLAW_HOME: "/srv/openclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { TINKERCLAW_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
      },
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of TINKERCLAW_HOME", () => {
    expect(
