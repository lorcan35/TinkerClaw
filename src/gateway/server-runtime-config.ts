...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set TINKERCLAW_GATEWAY_TOKEN/TINKERCLAW_GATEWAY_PASSWORD)`,
    );
  }
  if (
    controlUiEnabled &&
