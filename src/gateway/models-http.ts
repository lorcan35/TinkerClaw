...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([TINKERCLAW_MODEL_ID, TINKERCLAW_DEFAULT_MODEL_ID]);
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== TINKERCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
