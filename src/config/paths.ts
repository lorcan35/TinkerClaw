 ...
 * outside the config directory. Set via `TINKERCLAW_INCLUDE_ROOTS` as a
...
export function resolveIncludeRoots(
  ...
  const raw = env.TINKERCLAW_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.TINKERCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const openclawStateDir = env.TINKERCLAW_STATE_DIR?.trim();
  if (openclawStateDir) {
...
export function resolveGatewayPort(
  ...
  const envRaw = env.TINKERCLAW_GATEWAY_PORT?.trim();
