...
describe("image input pixel guard", () => {
  ...
  it("fails closed when sips cannot determine image dimensions", async () => {
    const previousBackend = process.env.TINKERCLAW_IMAGE_BACKEND;
    process.env.TINKERCLAW_IMAGE_BACKEND = "sips";
    try {
      await expect(
        resizeToJpeg({
      ...
      if (previousBackend === undefined) {
        delete process.env.TINKERCLAW_IMAGE_BACKEND;
      } else {
        process.env.TINKERCLAW_IMAGE_BACKEND = previousBackend;
      }
    }
  });
});
