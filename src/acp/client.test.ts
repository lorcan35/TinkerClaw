...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets TINKERCLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openclaw",
    });

    expect(env.TINKERCLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openclaw");
  });

  it("overrides pre-existing TINKERCLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      TINKERCLAW_SHELL: "wrong",
    });
    expect(env.TINKERCLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves TINKERCLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        TINKERCLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["TINKERCLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OpenClaw bridge", () => {
    ...
    expect(env.TINKERCLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.TINKERCLAW_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("TINKERCLAW_API_KEY")).toBe(false);
  });
});
