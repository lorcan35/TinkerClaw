...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openclawHome: process.env.TINKERCLAW_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("TINKERCLAW_HOME", snapshot.openclawHome);
  restoreKey("TINKERCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked TINKERCLAW_HOME.
  delete process.env.TINKERCLAW_HOME;
  process.env.TINKERCLAW_STATE_DIR = path.join(base, ".openclaw");
