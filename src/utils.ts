...
export function resolveConfigDir(
  ...
  const configPath = env.TINKERCLAW_CONFIG_PATH?.trim();
  if (configPath) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.TINKERCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$TINKERCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via TINKERCLAW_STATE_DIR.
