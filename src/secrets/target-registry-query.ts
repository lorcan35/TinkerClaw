...
const TINKERCLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of TINKERCLAW_COMPILED_SECRET_TARGETS) {
...
const TINKERCLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: TINKERCLAW_COMPILED_SECRET_TARGETS,
    entriesById: TINKERCLAW_TARGETS_BY_ID,
  });
