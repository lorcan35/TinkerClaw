...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "TINKERCLAW_ENV_TEST_A";
    const keyB = "TINKERCLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "TINKERCLAW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "TINKERCLAW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "TINKERCLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "TINKERCLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "TINKERCLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "TINKERCLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "TINKERCLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    const previousBundledDir = process.env.TINKERCLAW_BUNDLED_PLUGINS_DIR;
    process.env.TINKERCLAW_HOME = "/srv/openclaw-home";
    process.env.TINKERCLAW_STATE_DIR = "/srv/openclaw-state";
    process.env.TINKERCLAW_BUNDLED_PLUGINS_DIR = "/srv/openclaw-bundled";
    ...
    try {
      ...
      expect(env.TINKERCLAW_HOME).toBeUndefined();
      expect(env.TINKERCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.TINKERCLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("TINKERCLAW_HOME", previousOpenClawHome);
      restoreEnvKey("TINKERCLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("TINKERCLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processOpenClawHome: process.env.TINKERCLAW_HOME,
          processBundledDir: process.env.TINKERCLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.TINKERCLAW_BUNDLED_PLUGINS_DIR,
        }),
      );
      ...
      expect(process.env.TINKERCLAW_HOME).toBe("/srv/openclaw-home");
    } finally {
      restoreEnvKey("TINKERCLAW_HOME", previousOpenClawHome);
    }
  });
