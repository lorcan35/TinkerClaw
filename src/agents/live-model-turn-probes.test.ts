...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "TINKERCLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { TINKERCLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "TINKERCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { TINKERCLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "TINKERCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
