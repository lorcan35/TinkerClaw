...
const GOOGLE_LIVE_SINGLE_KEY = "TINKERCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "TINKERCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "TINKERCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "TINKERCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `TINKERCLAW_LIVE_${base}_KEY`;
