...
describe("sanitizeToolCallInputs redacts sessions_spawn attachments", () => {
  it("replaces attachments[].content with __TINKERCLAW_REDACTED__", () => {
    ...
    expect(tool?.arguments?.attachments?.[0]?.content).toBe("__TINKERCLAW_REDACTED__");
  ...
  it("replaces non-content attachment payload fields with a minimal redacted stub", () => {
    ...
    expect(attachment).toEqual({
      ...
      mimeType: "application/json",
      encoding: "utf8",
      content: "__TINKERCLAW_REDACTED__",
    });
    expect(JSON.stringify(out)).not.toContain(secret);
  });
  ...
  it("redacts ACP-only routing fields from arguments and input payloads", () => {
    ...
    expect(argumentTool?.arguments?.resumeSessionId).toBe("__TINKERCLAW_REDACTED__");
    expect(argumentTool?.arguments?.streamTo).toBe("__TINKERCLAW_REDACTED__");
    expect(inputTool?.input?.resumeSessionId).toBe("__TINKERCLAW_REDACTED__");
    expect(inputTool?.input?.streamTo).toBe("__TINKERCLAW_REDACTED__");
  ...
  it("redacts ACP-only routing fields with non-string payloads", () => {
    ...
    expect(tool?.input?.resumeSessionId).toBe("__TINKERCLAW_REDACTED__");
    expect(tool?.input?.streamTo).toBe("__TINKERCLAW_REDACTED__");
