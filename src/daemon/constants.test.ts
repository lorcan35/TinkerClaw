...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { TINKERCLAW_PROFILE: "work", TINKERCLAW_SERVICE_VERSION: "1.0.0" },
        description: "Custom",
      }),
    ).toBe("Custom");
  });
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { TINKERCLAW_PROFILE: "work", TINKERCLAW_SERVICE_VERSION: "local" },
        environment: { TINKERCLAW_SERVICE_VERSION: "remote" },
      }),
