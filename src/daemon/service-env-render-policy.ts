...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.TINKERCLAW_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.TINKERCLAW_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.TINKERCLAW_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  if (!isLaunchAgentServiceEnvironment(params)) {
    return;
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    TINKERCLAW_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
