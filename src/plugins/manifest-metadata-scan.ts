...
const TINKERCLAW_PACKAGE_ROOT = fileURLToPath(new URL("../..", import.meta.url));
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.TINKERCLAW_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.TINKERCLAW_STATE_DIR);
  if (override) {
...
function areBundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const value = normalizeTrimmedString(env.TINKERCLAW_DISABLE_BUNDLED_PLUGINS)?.toLowerCase();
...
function resolveBundledPluginRoot(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = normalizeTrimmedString(env.TINKERCLAW_BUNDLED_PLUGINS_DIR);
  if (override) {
    return resolveUserPath(override, env);
  }

  const sourceRoot = path.join(TINKERCLAW_PACKAGE_ROOT, "extensions");
  const runtimeRoot = path.join(TINKERCLAW_PACKAGE_ROOT, "dist-runtime", "extensions");
  const distRoot = path.join(TINKERCLAW_PACKAGE_ROOT, "dist", "extensions");
