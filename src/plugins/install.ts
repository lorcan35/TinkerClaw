...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_TINKERCLAW_EXTENSIONS: "missing_openclaw_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_TINKERCLAW_EXTENSIONS: "empty_openclaw_extensions",
  INVALID_TINKERCLAW_EXTENSIONS: "invalid_openclaw_extensions",
...
function ensureOpenClawExtensions(params: { manifest: PackageManifest }):
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_TINKERCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_TINKERCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
> {
  ...
  if (!minHostVersionCheck.ok) {
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires OpenClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set TINKERCLAW_VERSION and retry.`,
  ...
  if (!extensionValidation.ok) {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_TINKERCLAW_EXTENSIONS,
    };
  }
